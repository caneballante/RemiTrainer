...
async function spinUpWorkout() {
  ...
  try {
    ...
    const originalAiResponse = buildStrictJsonResponse(request, compactContext, parentPlan);
...
function saveGeneratedWorkout(request, compactContext, originalAiResponse, finalPlan) {
  const session = {
    id: id("session"),
